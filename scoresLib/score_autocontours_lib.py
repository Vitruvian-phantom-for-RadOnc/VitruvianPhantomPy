...
def load_dictionary_of_structure(structure_dictionary_filename):
    with open(structure_dictionary_filename) as f:
        structure_dictionary_file = json.load(f)

    # Read the dictionary
    structure_dictionary = structure_dictionary_file.get('Naming')
...
def get_distance_measures(ref_poly, test_poly, stepsize=1.0, warningsize=1.0):
    ...
    :param warningsize:
    :return: a list of distances along the polygones ref_to_test, test_2_ref
    """
...
def estimate_slice_thickness(contour_data_set, debug=False):
    """
    Estimate the slice thikness without the CT image
...
def score_a_case(reference_rtss_filename, test_rtss_filename, slice_thickness=0,
               use_roi_name=False, verbose=False, tolerance=1.0):
    """
...
def format_result_list(result_list):
    ...
    for result in result_list:
        ...
        results_structure = {'RefOrgan': ref_name, 'TestOrgan': test_name,
                             ...
                             'HD_ref2test': scores[8], 'HD_test2ref': scores[9], 'HD': max(scores[8], scores[9]),
                             'HD95_ref2test': scores[10], 'HD95_test2ref': scores[11], 'HD95': max(scores[10], scores[11]),
                             'AD_ref2test': scores[12], 'AD_test2ref': scores[13], 'AD': (scores[12] + scores[13])/2.0,
                             'MD_ref2test': scores[14], 'MD_test2ref': scores[15], 'MD': max(scores[14], scores[15]),
                             'ref_cent': scores[16], 'test_cent': scores[17],
                             'cent_dist': np.linalg.norm(scores[16] - scores[17])}
